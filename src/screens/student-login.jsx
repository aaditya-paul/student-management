...
export function StudentLogin({token}) {
    ...
    const validateToken = async () => {
      if (token && token !== "undefined") {
        ...
        if (tokenSnap.exists()) {
          ...
          if (tokenData.type === "student" && tokenData.isActive) {
            setValidToken(true);
          } else {
            setError("This link is not valid for student access.");
          }
        } else {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (validToken) {
        ...
        router.push(`/student/details?token=${token}&uid=${user.uid}`);
        // router.push("/redirect");
        setLoading(false);
        return;
      }

      // No token: check if student exists in student-invitations
      const docRef = doc(db, "student-invitations", email);
